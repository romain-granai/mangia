<?php
function content_blocks($page_id = null) {
    ?>
    <?php 
            // If no ID is passed, use the current post ID
    if (!$page_id) {
        $page_id = get_the_ID();
    }

    ?>
    <?php if (have_rows('blocks', $page_id)): ?>
        <?php while (have_rows('blocks', $page_id)): the_row(); ?>
            ...
            <?php elseif (get_row_layout() == 'pattern_title'):
                ...
                $blockSizeClass = get_sub_field('block_size') == 'small' ? 'block--title-pattern--sm' : ''; 
            ?>
                <?php if ($isH1): ?>
                    <header class="block block--title-pattern <?php echo $blockSizeClass; ?>">
                <?php else: ?>
                    <div class="block block--title-pattern <?php echo $blockSizeClass; ?>">
                <?php endif; ?>
