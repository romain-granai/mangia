...
<head>
  ...
  <meta name="theme-color" content="FFD12D">
	<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-KG23CEMK9G"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-KG23CEMK9G');
</script>
</head>
...
<body <?php body_class(); ?> data-barba="wrapper">
    ...
    <?php if(! is_account_page() && ! is_cart() && ! is_checkout() && ! is_product() && ! is_shop()): ?>
      <div class="curtain">
      ...
      <?php
        ...
        if (!empty($rightMenuArray)) {
          ...
          foreach ($rightMenuArray as $menuItem) {
              echo '<li><a href="' . $menuItem['url'] . '" title="' . $menuItem['title'] . '">' . $menuItem['title'] . '</a>';
              
              // Check if the current menu item has sub-items (children)
              if (!empty($menuItem['children'])) {
                  echo '<ul>'; // Start the submenu
                  foreach ($menuItem['children'] as $subMenuItem) {
                      echo '<li><a href="' . $subMenuItem['url'] . '" title="' . $subMenuItem['title'] . '">' . $subMenuItem['title'] . '</a></li>';
                  }
                  echo '</ul>'; // End the submenu
              }
